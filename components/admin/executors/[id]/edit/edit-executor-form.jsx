...
// Admin tomonidan anketani tahrirlash (334:14442).
export default function AdminEditExecutor({ id }) {
    ...
    async function submit() {
        const res = await patchDataToken(`/admin/executors/${id}/`, form)
        if (res.success) {
            toast.success('Изменения сохранены')
            router.push(`/admin/executors/${id}`)
        } else {
    ...
    return (
        <FormCard
            ...
            description="Изменения применяются сразу, без повторной модерации."
            onSubmit={submit}
