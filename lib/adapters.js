...
export function performerCard(item) {
    ...
    return {
        id: item.id,
        // Backend slug bermaydi — marshrutlarda UUID ishlatiladi.
        slug: item.id,
        ...
        // Katalogda yo'nalishlar kelmaydi — kartochkadagi teglar bo'sh qoladi.
        tags: item.work_directions || item.tags || [],
...
export function venueCard(item) {
    ...
    return {
        ...
        city: item.city || '',
        type: item.venue_type || item.category || '',
...
export function venueDetail(data) {
    ...
    return {
        ...
        slug: data.id,
        name: data.name || '',
        type: data.venue_type || data.category || '',
...
export function projectCard(item) {
    ...
    return {
        ...
        likes: item.likes_count ?? 0,
        status: item.status,
        requirements: item.requirement_tags || [],
...
export function adminDashboard(data) {
    ...
    return {
        ...
        users: (data.latest_users || []).map((u) => ({
            id: u.id,
            name: adminName(u),
            type: ENTITY_TITLE[u.performer_specialty || u.role] || '—',
            email: u.email || '—',
...
export function adminUserRow(item) {
    ...
    return {
        id: item.id,
        name: adminName(item),
        type: ENTITY_TITLE[item.performer_specialty || item.customer_type || item.role] || '—',
        email: item.email || '—',
        phone: item.phone || '—',
        city: item.city || '—',
...
export function adminCustomerProfile(data) {
    ...
    return {
        ...
        // «Сфера деятельности» — Figma 338:16487. Backendda zakazchik uchun
        // bunday maydon yo'q (CustomerWriteRequest), so'ralgan — hisobot 15-band.
        field: user.sphere_of_activity || user.business_field || '',
