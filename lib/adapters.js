...
export function performerCard(item) {
    ...
    return {
        ...
        // Fotograf/videograf kartochkasidagi chiplar («9 лет опыта»,
        // «31 съёмок»). Anketada bu qiymatlar `profile` ichida, ro'yxatda esa
        // tekis keladi.
        experienceYears: item.years_experience ?? null,
        shoots: item.shoots_count ?? null,
        cases: item.cases_count ?? item.brands_count ?? null,
...
function performerStats(profile = {}, specialty) {
    // Plitkada katta son va uning tagida izoh turadi (Figma 136:7590), shuning
    // uchun izoh son bilan kelishishi kerak: «1 Съёмка», «3 Съёмки», «7 Съёмок».
    const pick = (value, one, few, many) =>
        value === null || value === undefined
            ? null
            : { value: String(value), label: capitalize(plural(value, one, few, many)) }

    const base =
        specialty === EXECUTOR_TYPES.MODEL
            ? [
                  pick(profile.years_experience, 'год опыта', 'года опыта', 'лет опыта'),
                  pick(profile.shoots_count, 'съёмка', 'съёмки', 'съёмок'),
                  pick(profile.brands_count, 'бренд', 'бренда', 'брендов'),
                  pick(profile.projects_count, 'проект', 'проекта', 'проектов'),
              ]
            : [
                  pick(profile.years_experience, 'год опыта', 'года опыта', 'лет опыта'),
                  pick(profile.shoots_count, 'съёмка', 'съёмки', 'съёмок'),
                  pick(profile.cases_count ?? profile.brands_count, 'кейс', 'кейса', 'кейсов'),
                  pick(
                      profile.videos_count ?? profile.projects_count,
                      'проект',
                      'проекта',
                      'проектов',
                  ),
...
// Ijrochi qaysi agentlikda turishi — anketadagi yorliq uchun (Figma 345:19391).
//
// Backend uchta ko'rinishning istalganini berishi mumkin, uchalasi ham
// qo'llab-quvvatlanadi:
//   agency: { id, slug, name, logo_url }        ← afzal
//   agency: { id, slug, agency_name, logo_url }
//   tekis maydonlar: agency_id · agency_slug · agency_name · agency_logo_url
//
// Hech qaysi kelmasa — `null`, yorliq chizilmaydi.
function performerAgency(data = {}, user = {}) {
    const a = data.agency || user.agency || {}
    const id = a.id || data.agency_id || user.agency_id || null
    const name = a.name || a.agency_name || data.agency_name || user.agency_name || ''
    if (!id && !name) return null
    // «Ещё 11 исполнителей» (Figma 345:19395) — agentlikdagi umumiy son.
    // Joriy anketa ham shu songa kiradi, shuning uchun yorliqda bittasi
    // ayriladi. Backend sonni bermasa yorliqda oddiy «Агентство» turadi.
    const performers =
        a.performers_count ?? a.stats?.performers ?? data.agency_performers_count ?? null

    return {
        id,
        slug: a.slug || data.agency_slug || user.agency_slug || id,
        name: name || 'Агентство',
        logo: cover(a.logo_url || data.agency_logo_url || user.agency_logo_url),
        performersCount: performers,
    }
}
...
export function performerDetail(data) {
    ...
    return {
        ...
        // Agentlik yorlig'i (Figma 345:19391) — backend maydon bersa chiqadi.
        agency: performerAgency(data, user),
...
export function venueDetail(data) {
    ...
    // «Характеристики» — Figma 138:8419. Chap ustun: Площадь · Высота потолков ·
    // Вместимость · Количество залов · Естественный свет; o'ng ustun:
    // Минимальная аренда · Этаж · Лифт · Парковка · Грузовой вход · Город.
    // Ya'ni makette «Есть/Нет» bayroqlari ham shu ro'yxat ichida turadi.
    // Faqat to'ldirilgan maydonlar chiqadi.
    const params = []
    const push = (label, value, suffix = '') => {
        if (value === null || value === undefined || value === '') return
        params.push([label, `${value}${suffix}`])
    }
    // Bayroq: `true` bo'lsa «Есть», `false` bo'lsa «Нет», berilmagan bo'lsa —
    // qator umuman chizilmaydi.
    const pushFlag = (label, value) => {
        if (value === null || value === undefined) return
        params.push([label, value === true ? 'Есть' : 'Нет'])
    }
    ...
    pushFlag('Естественный свет', data.natural_light)
    push('Минимальная аренда', data.min_rental_label)
    push('Этаж', data.floor)
    push('Окна', data.windows_count)
    pushFlag('Лифт', data.elevator)
    pushFlag('Парковка', data.parking)
    pushFlag('Грузовой вход', data.freight_entrance)
...
export function chatListItem(c) {
    if (!c) return null
    // `/site/chats` suhbatdoshni `peer_*` bilan beradi, `/admin/chats` esa
    // `role` va `user_a_id`/`user_b_id` bilan — ikkala shakl ham o'qiladi.
    const role = c.peer_role || c.role || null
    const name =
        c.agency_name ||
        c.company_name ||
    ...
    return {
        id: c.id,
        // Texnik yordam suhbati — adminka uni oddiy yozishmadan ajratadi.
        kind: c.kind || 'direct',
        userAId: c.user_a_id || null,
        userBId: c.user_b_id || null,
        peerId: c.peer_id || null,
        peerRole: role,
        slug: c.slug || null,
        ...
        companion: {
            ...
            role: AUTHOR_TYPE[c.specialty] || AUTHOR_TYPE[role] || '',
        },
        name,
        avatar: c.logo_url || null,
...
// Ochiq zakazchik profili — GET /site/customers/{id} (Figma «Профиль
// частное лицо» 173:3550 va «Профиль компании» 171:2745).
//
// Maydon kartochkasidagi egasi shu sahifaga olib boradi. Ruchka faqat UUID
// qabul qiladi (slug bilan 422), shuning uchun havolada ham `owner.id` turadi.
export function personDetail(data) {
    ...
    return {
        ...
        phone: data.contact_phone || user.phone || '',
        inn: data.inn || '',
        // Sahifadagi kartochkalar `components/shared/*` dan — ular rasmni
        // `cover` deb kutadi, adapterlar esa `image` beradi.
        projects: (data.projects || [])
            .map(projectCard)
            .filter(Boolean)
            .map((p) => ({ ...p, cover: p.image })),
        venues: (data.venues || [])
            .map(venueCard)
            .filter(Boolean)
            .map((v) => ({ ...v, cover: v.image })),
