...
// Figma: активен (216:5469) / отклонен (216:5737).
const TABS = [
...
export default function CompanyProjectDetail({ id }) {
    ...
    if (!project) {
        return <EmptyState title="Проект не найден" actionText="К проектам" actionHref="/company/projects" />
    }
        ...
        <div className="flex flex-col gap-6">
            ...
            <Card
                ...
                action={
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Button href="/company/executors" variant="whiteStroke" size="sm">
            ...
            >
                ...
                <dl className="mt-6 grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="flex justify-between gap-4">
                        <dt className="text-sm text-grey">Площадка</dt>
                        <dd className="text-base text-black">{project.venue?.name || '—'}</dd>
                    </div>
                    <div className="flex justify-between gap-4">
            ...
            <Card title="Исполнители" padded={false} className="border-0 bg-transparent">
                ...
                {people.length === 0 ? (
                    <EmptyState
                        title="Пока никого"
                        description="Пригласите исполнителей из каталога."
                        actionText="В каталог"
                        actionHref="/company/executors"
                    />
                ) : (
                    <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 lg:gap-6">
                        {people.map((e) => (
                            <ExecutorCard key={e.id} executor={e} basePath="/company/executors" />
                        ))}
                    </div>
                )}
            </Card>
