...
## 1. `venue_type` отдаёт чужие типы
...
Ждём только те, у кого `venue_type` равен запрошенному слагу.
Фильтр «Тип площадки» показывает не то, что выбрали.
...
## 2. `performers_count_max` в проектах

`performers_count` — только нижняя граница (`model_count >= value`), верхней нет:

```
GET /site/projects?performers_count=1&performers_count_max=2
  → 13 проектов, model_count 1…5     ✘ max игнорируется
```

Пункт «От 3 до 5 человек» без верхней границы не выразить.
...
| Параметр | Как читаем |
|---|---|
| `performers_count` | включительно: `model_count >= min` |
| `performers_count_max` | включительно: `model_count <= max` |

Мы уже отправляем обе границы — как только появится, заработает само.

---

## 3. `agency.performers_count` — подпись на плашке
...
```json
{
  "agency": {
    ...
    "slug": "u-b0c314c57273",
    "name": "Star Agency",
    "logo_url": "https://admin.modelworkrf.ru/uploads/demo/p-agency.jpg",
    "performers_count": 4
  }
}
```
...
## 4. `GET /admin/chats` — нет названия агентства
...
**Одинаковые счётчики.** У всех фотографов и видеографов `cases_count = 15`
и `brands_count = 12`:
...
**Обложки проектов — 1.9 МБ PNG.**

```
GET https://admin.modelworkrf.ru/uploads/projects/cover-1.png → 1 934 085 байт, 1413×1113
GET https://admin.modelworkrf.ru/uploads/projects/cover-2.png → 1 777 079 байт, 1536×1024
```

Остальные демо-картинки вы уже пересохранили в JPEG по ~150 КБ — эти две
остались в PNG. На карточке они показываются в 384px. Достаточно пересохранить.

---

## Что делать не нужно

Формат ответа, словари, фильтры исполнителей, приглашения, модерация, чаты —
всё сходится. Пункты отчётов №5–№10 закрыты, повторно проверять не нужно.
