...
// ─────────────────────────────────────────────────────────────────────────────
// Kartochka chiplari: «9 лет опыта» · «31 съёмка» · «45 кейсов».
// Figma'da bitta misol turadi (138:8181, 138:8183), backend esa istalgan son
// qaytaradi — shuning uchun qo'shimchani har safar shu yerda tanlaymiz.
// ─────────────────────────────────────────────────────────────────────────────
export function experienceLabel(years) {
    if (years == null) return ''
    return `${years} ${plural(years, 'год', 'года', 'лет')} опыта`
}

export function shootsLabel(count) {
    if (count == null) return ''
    return `${count} ${plural(count, 'съёмка', 'съёмки', 'съёмок')}`
}

export function casesLabel(count) {
    if (count == null) return ''
    return `${count} ${plural(count, 'кейс', 'кейса', 'кейсов')}`
}
