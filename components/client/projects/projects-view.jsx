'use client'

import React from 'react'
import Card from '@/components/ui/card'
import ResourceList from '@/components/cabinet/resource-list'
import { toProjectCard, customerProjects } from '@/components/cabinet/list-fetchers'
import ProjectCard from '@/components/shared/project-card'

const TABS = [
        { label: 'Активные', value: 'active' },
        { label: 'На модерации', value: 'moderation' },
        { label: 'Отклонённые', value: 'rejected' },
        { label: 'Завершённые', value: 'completed' },
    ]

export default function ClientProjects() {
    return (
        <Card title="Мои проекты" padded={false} className="border-0 bg-transparent">
            <ResourceList
                fetcher={customerProjects}
                adapt={toProjectCard}
                tabs={TABS}
            createText="Создать проект"
            createHref="/client/projects/new"
                columns="grid-cols-1 md:grid-cols-2"
                emptyTitle="Проектов пока нет"
                emptyDescription="Создайте проект — исполнители смогут откликнуться."
                renderItem={(item) => <ProjectCard key={item.id} project={item} basePath="/client/projects" />}
            />
        </Card>
    )
}
